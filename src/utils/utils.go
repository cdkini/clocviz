...
func RunCloc(in string, out string, gitObj string) error {
	...
	cloc := exec.Command("cloc", in, "--json", "--by-file-by-lang", fmt.Sprintf("--out=%s", out), "--git", gitObj)
	if err := cloc.Run(); err != nil {
